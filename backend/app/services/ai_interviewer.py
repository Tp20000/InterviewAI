...
class AIInterviewer:
    ...
    def generate_greeting(self, candidate_name, role_name, company_name):
        messages = [
            ...
            {"role": "user", "content": (
                ...
                "Welcome by first name, introduce yourself as Alex, mention role, ask them to introduce themselves. 3-4 sentences max."
            )}
        ]
        return self._chat(messages, temperature=0.8, max_tokens=200)

    def generate_next_question(self, role_name, company_name, topics, conversation_history,
                                current_topic, question_number, total_questions,
                                experience_level, last_answer="", resume_text="", jd_text=""):
        ...
        system_prompt = (
            ...
            "- If candidate mentioned projects/skills in resume, ask about them\n"
            "- For technical roles, occasionally ask DSA/problem-solving questions\n"
            "- Each candidate gets unique questions based on their background\n"
        ...
        if resume_text:
            user_content += "\n\nCandidate Resume (personalize questions from this):\n" + resume_text[:1500]
        if jd_text:
            user_content += "\n\nJob Description:\n" + jd_text[:800]
    ...
    def generate_closing(self, candidate_name, role_name):
        messages = [
            ...
            {"role": "user", "content": "Generate 2-3 sentence closing for " + candidate_name + " who finished interviewing for " + role_name + ". Thank them, mention results will be shared."}
        ]
    ...
    def evaluate_answer(self, question, answer, topic, role_name, experience_level):
        if not answer or len(answer.strip()) < 10:
            return {"technical_score":0,"relevance_score":0,"clarity_score":0,"depth_score":0,
                    "overall_score":0,"feedback":"No meaningful answer.","is_ai_generated":False}

        prompt = (
            "Role: " + role_name + " (" + experience_level + ")\n"
            "Topic: " + topic + "\nQuestion: " + question + "\nAnswer: " + answer + "\n\n"
            "Score 0-10 each. Reply JSON only:\n"
            '{"technical_score":<0-10>,"relevance_score":<0-10>,"clarity_score":<0-10>,"depth_score":<0-10>,"feedback":"<2 sentences>","is_ai_generated":<true/false>}'
        )
        messages = [
        ...
        try:
            ...
            if start >= 0 and end > start:
                ...
                data["overall_score"] = round((ts*0.40)+(rs*0.25)+(cs*0.20)+(ds*0.15), 2)
                return data
        except Exception:
            pass
        return {"technical_score":5,"relevance_score":5,"clarity_score":5,"depth_score":5,
                "overall_score":5,"feedback":"Evaluated.","is_ai_generated":False}

    def generate_transition(self, quality="good"):
        opts = {"good":["Great answer!","Excellent!","Very insightful!"],
