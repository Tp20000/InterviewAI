...
const GradeBadge = ({ score }) => {
  ...
  return (
    <span className={"text-base font-black px-2.5 py-1 rounded-xl border " + cls}>
      {g}
    </span>
  )
}
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 mb-8">
        {[1,2,3,4,5].map(i => (
          <div key={i} className="bg-slate-800 border border-slate-700 rounded-2xl p-4 animate-pulse">
            ...
            <div className="h-7 bg-slate-700 rounded w-12 mx-auto mb-1" />
    ...
    try {
      ...
      sock.on("new_invitation", (data) => {
        toast.success(
          ...
          (data.interview_title || "Interview") +
          " at " + (data.company_name || "Company"),
          { duration: 6000 }
        )
        loadData()
      })
      ...
      <div className="max-w-6xl mx-auto px-4 md:px-6 py-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
          ...
          <Link to="/mock" className="btn-primary px-6 py-3 text-sm w-fit">
            <I name="target" size={4} className="text-white" /> Practice Mock
          </Link>
        </div>
        ...
        <div className="card">
          <div className="flex items-center gap-2 mb-6 overflow-x-auto pb-1">
            ...
            <button onClick={loadData}
              className="ml-auto btn-secondary text-xs py-1.5 px-3 flex-shrink-0">
          ...
          {display.length === 0 ? (
            <div className="text-center py-16">
              <I name={tab === "upcoming" ? "calendar" : "clipboard"}
              ...
              <p className="text-slate-400 text-sm mb-6">
                {tab === "upcoming"
                  ? "A company will invite you. Try mock practice!"
                  : "Complete interviews to see results here."}
              </p>
              {tab !== "completed" && (
                <Link to="/mock" className="btn-primary px-6">
                  Try Mock Interview
                </Link>
              )}
            </div>
                ...
                <div key={i}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      ...
                      {iv.is_mock && <span className="badge-purple text-xs">Mock</span>}
                    </div>
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs text-slate-400">
                      ...
                      <span className="flex items-center gap-1">
                        <I name="chart" size={3} /> {iv.interview?.experience_level}
                      </span>
                      <span className="flex items-center gap-1">
        ...
        {interviews.length === 0 && (
          <div className="mt-6 grid grid-cols-1 md:grid-cols-3 gap-4">
            {[
              { icon: "target", title: "Practice First",
                desc: "Try a mock interview to get comfortable with the AI format." },
              { icon: "mic",    title: "Use Voice Input",
                desc: "Speak your answers naturally. AI understands speech clearly." },
              { icon: "eye",    title: "Stay Visible",
                desc: "Keep your face in the camera throughout the interview." }
            ].map((tip, i) => (
              <div key={i}
